...
CPropertyPage::CPropertyPage(int objectIndex)
	: m_refCount(1), 
    ...
    if (m_hwnd) 
	{
        ...
        if (m_fDirty) 
		{
            ...
            if (m_pPropertyPageSite)
                m_pPropertyPageSite->OnStatusChange(PROPPAGESTATUS_VALIDATE);
        }
    } 
	else
        return E_UNEXPECTED;
...
{
    MAKE_TCHARPTR_FROMWIDE(szHelpDir, (LPOLESTR)pszHelpDir);
    BOOL f = WinHelp(m_hwnd, szHelpDir, HELP_CONTEXT, HELPCONTEXTOFPROPPAGE(0));
...
HRESULT CPropertyPage::m_EnsureLoaded(void)
{
    ...
    CreateDialog(GetResourceHandle(), 
				 TEMPLATENAMEOFPROPPAGE(0), 
				 GetParkingWindow(),
...
void CPropertyPage::m_ReleaseAllObjects(void)
{
	...
	HRESULT hResult;
    if (IsWindow(m_hwnd))
		SendMessage(m_hwnd, PPM_FREEOBJECTS, 0, (LPARAM)&hResult);

    // loop through and blow them all away.
    for (UINT x = 0; x < m_cObjects; x++)
